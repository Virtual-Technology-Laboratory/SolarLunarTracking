...
public class TimeSlider : MonoBehaviour
{
	...
	void Update () {
		if (IsPlaying) {
			if (LastPlayState)
			{
                targetSlider.value += (float)(DateTime.UtcNow - LastUpdate).TotalSeconds * PlaySpeed;
			}
			LastUpdate = DateTime.UtcNow;
			LastPlayState = true;

            SimStepProperty = (Int64)Math.Floor(targetSlider.value);
			SimTimeProperty = StartTime + TimeSpan.FromTicks( (Int64)((double)TimeDelta.Ticks * targetSlider.value));
		}
		else 
		{
			LastPlayState = false;
...
#if DEBUG
        Debug.Log(String.Format("{0}, {1}, {2}, {3}, {4}", IsPlaying, Time.time, targetSlider.value, SimStep, SimTime));
#endif
